...
import { TrendingUp, Building2, Package, DollarSign, Target, BarChart3, Filter } from 'lucide-react';
...
interface BranchKPIProps {
  invoices: Invoice[];
}

export const BranchKPI: React.FC<BranchKPIProps> = ({ invoices }) => {
  ...
  const filteredInvoices = useMemo(() => {
    return invoices.filter(invoice => {
      ...
      const matchesBranch = filters.branches.length === 0 ||
        filters.branches.includes(invoice.branch);

      return matchesDateRange && matchesBranch;
    });
  }, [invoices, filters]);
  ...
  const branchStats = useMemo(() => {
    ...
    filteredInvoices.forEach(invoice => {
      ...
      const current = stats.get(branch) || {
        name: branch,
        totalAmount: 0,
        totalBoxes: 0,
        totalBalance: 0,
      ...
      if (invoice.salesperson) {
        current.salespeople.add(invoice.salesperson);
      }
    ...
    return Array.from(stats.values()).map(stat => ({
      ...stat,
      salespeople: stat.salespeople.size,
    }));
  }, [filteredInvoices]);
  ...
  const totalStats = useMemo(() => {
    return branchStats.reduce((acc, curr) => ({
      ...
      totalSalespeople: acc.totalSalespeople + curr.salespeople,
    }), {
      totalAmount: 0,
      totalBoxes: 0,
      totalBalance: 0,
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <Filter size={16} />
            Filters
            {(filters.branches.length > 0 || filters.dateFrom || filters.dateTo) && (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-sm text-gray-500">
                {totalStats.totalSalespeople} total salespeople
              </p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center gap-2 text-yellow-600">
                ...
                <h3 className="font-medium">Average Per Branch</h3>
              </div>
              <p className="mt-2 text-2xl font-bold text-gray-900">
                {formatCurrency(totalStats.totalAmount / (branchStats.length || 1))}
              </p>
              <p className="text-sm text-gray-500">
                {(totalStats.totalInvoices / (branchStats.length || 1)).toFixed(1)} invoices avg.
              </p>
            </div>
          </div>

          {/* Leaderboard */}
          <Leaderboard branches={branchStats} />

          {/* Company KPI */}
          <CompanyKPI currentAmount={totalStats.totalAmount} currentBoxes={totalStats.totalBoxes} />

          {/* Individual Branch KPIs */}
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {branchStats.map((branch) => (
                <KPITargets
                  ...
                  currentAmount={branch.totalAmount}
                  currentBoxes={branch.totalBoxes}
