...
import { Target, Plus, Edit2, Save, X, DollarSign, Package, Users } from 'lucide-react';
...
export const KPITargets: React.FC<KPITargetsProps> = ({
  branchName,
  currentAmount,
  currentBoxes,
  salespeople,
  ...
  const currentTarget = settings.kpiTargets.find(t => 
    t.type === 'branch' && t.branchName === branchName
  );
  ...
  const handleSave = async () => {
    try {
      ...
      setIsEditing(false);
      toast.success('Branch KPI targets updated successfully');
    } catch (error) {
      toast.error('Failed to update branch KPI targets');
    }
  };
      ...
      <div className="bg-white rounded-lg shadow p-6 space-y-4">
        <div className="flex justify-between items-center">
          <h3 className="text-lg font-medium text-gray-900 flex items-center gap-2">
            ...
            Set KPI Targets for {branchName}
          </h3>
    ...
    <div className="bg-white rounded-lg shadow p-6 space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-medium text-gray-900 flex items-center gap-2">
            <Target className="h-5 w-5 text-blue-600" />
            {branchName}'s KPI Progress
          </h3>
          <div className="flex items-center gap-2 mt-1 text-sm text-gray-500">
            <Users className="h-4 w-4" />
            <span>{salespeople} active salespeople</span>
          </div>
        </div>
        <button
          onClick={() => {
