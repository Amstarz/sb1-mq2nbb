...
export const KPIFilters: React.FC<KPIFiltersProps> = ({
  ...
  const handleSalespersonToggle = (salespersonName: string) => {
    setFilters(prev => ({
      ...prev,
      salespeople: prev.salespeople.includes(salespersonName)
        ? prev.salespeople.filter(name => name !== salespersonName)
        : [...prev.salespeople, salespersonName]
    }));
  };
            ...
            {settings.salespeople
              ...
              .map((person) => (
                <label key={person.id} className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    checked={filters.salespeople.includes(person.value)}
                    onChange={() => handleSalespersonToggle(person.value)}
                    className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="text-sm text-gray-700">{person.value}</span>
                </label>
              ))}
          </div>
        </div>
