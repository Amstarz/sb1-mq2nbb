...
export const CompanyKPI: React.FC<CompanyKPIProps> = ({ currentAmount, currentBoxes }) => {
  ...
  const currentTarget = settings.kpiTargets.find(t => t.type === 'company-sales');
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm">
          ...
          <span className="text-gray-500">
            {remaining > 0 
              ? `${type === 'money' ? formatCurrency(remaining) : remaining} to go` 
              : 'Target achieved!'}
          </span>
        </div>
        ...
        <div className="flex justify-between text-xs text-gray-500">
          <span>Current: {type === 'money' ? formatCurrency(current) : current}</span>
          <span>Target: {type === 'money' ? formatCurrency(target) : target}</span>
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Company Daily Target (MYR)</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Company Weekly Target (MYR)</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Company Monthly Target (MYR)</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Company Box Target</label>
            <input
              type="number"
              value={targets.boxTarget}
              onChange={(e) => setTargets(prev => ({ ...prev, boxTarget: parseInt(e.target.value) || 0 }))}
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
              min="0"
            />
          </div>
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <button
          onClick={() => {
            setTargets({
              ...
              boxTarget: currentTarget?.boxTarget || 0,
            });
            setIsEditing(true);
          }}
      ...
      <div className="space-y-6">
        {currentTarget ? (
          <>
            {renderProgressBar('Company Daily Target', currentAmount, currentTarget.dailyTarget, 'money')}
            {renderProgressBar('Company Weekly Target', currentAmount, currentTarget.weeklyTarget, 'money')}
            {renderProgressBar('Company Monthly Target', currentAmount, currentTarget.monthlyTarget, 'money')}
            {renderProgressBar('Company Box Target', currentBoxes, currentTarget.boxTarget, 'boxes')}
          </>
        ) : (
